...
fn compile_type(typ: &Type, ctx: TypeKind) -> Result<TokenStream2, Error> {
    ...
    Ok(match ctx {
        TypeKind::Owned => quote! { <#typ as ::exec_spec_lib::ExecSpecType>::ExecOwnedType },
        TypeKind::Ref => quote! { <#typ as ::exec_spec_lib::ExecSpecType>::ExecRefType<'_> },
    })
}
...
fn compile_struct(item_struct: &ItemStruct) -> Result<TokenStream2, Error> {
    ...
    Ok(quote! {
        ...
        impl ::exec_spec_lib::ExecSpecType for #spec_name {
            type ExecOwnedType = #exec_name;
            type ExecRefType<'a> = &'a #exec_name;
        }

        impl<'a> ::exec_spec_lib::ToRef<&'a #exec_name> for &'a #exec_name {
            fn get_ref(self) -> &'a #exec_name {
                self
            }
        }

        impl<'a> ::exec_spec_lib::ToOwned<#exec_name> for &'a #exec_name {
        ...
        impl ::exec_spec_lib::DeepViewClone for #exec_name {
    ...
    let expr_ts = match expr {
        ...
        Expr::Binary(expr_binary) => match &expr_binary.op {
            ...
            BinOp::Eq(..) => {
                ...
                quote! { ::exec_spec_lib::ExecSpecEq::exec_eq(#left, #right) }
            }
